...
from generator.embedding import (
    Embedding,
)  # 直接在def里面包含了输入数据的向量化,便于faiss数据库的查询,这个东西在主程序只有一个作用,就是向量化输入然后开始索引
import toml
...
class ResponseGenerator:
        ...
        relevant_texts = list(
            set(
                [
                    ...
                    for i in (
                        unique_id_laws[0]
                        if isinstance(unique_id_laws[0], list)
                        else [unique_id_laws[0]]
                    )
                ]
            )
        )
    ...
    # 日常寒暄输出
    def generate_response_dailys(
        self, input_user_id, input_session_id, question, return_template=False
    ):
        role = f"""
        ...
        try:
            ...
            history_control.update_session_history(
                input_user_id, input_session_id, question, answer.content
            )  # 更新历史记录
            if return_template:
    ...
    def generate_response_laws(
        ...
        def generate_law_answer():
            ...
            try:
                law_answer = chat_with_history_law.invoke(
                    {"question": question},
                    config={
                        "input_user_id": input_user_id,
                        "input_session_id": input_session_id,
                ...
                print(
                    f"在 generate_response_laws 函数的 generate_law_answer 中调用模型时出错: {e}"
                )
                return None
            history_control.update_session_history(
                input_user_id, input_session_id, question, law_answer.content
            )
            return law_answer
        ...
        def generate_case_info():
            ...
            try:
                case_info = chat_with_history_case.invoke(
                    {"question": question},
                    config={
                        "input_user_id": input_user_id,
                        "input_session_id": input_session_id,
                ...
                print(
                    f"在 generate_response_laws 函数的 generate_case_info 中调用模型时出错: {e}"
                )
                return None
            history_control.update_session_history(
                input_user_id, input_session_id, question, case_info.content
            )
            return case_info
        ...
        try:
            final_answer = chat_with_history_final.invoke(
                {"question": question},
                config={
                    "input_user_id": input_user_id,
                    "input_session_id": input_session_id,
        ...
        if final_answer is not None:
            history_control.update_session_history(
                input_user_id, input_session_id, question, final_answer.content
            )
    ...
    def analyze_case_with_law(
        ...
        def generate_law_analysis():
            ...
            <|history|>上下文记录:{{history}}<|end|>
            """
            prompt_law_analysis = ChatPromptTemplate.from_template(
                template_law_analysis
            )
            history_control = ControlChatHistoryData()
            ...
            try:
                law_analysis = chat_with_history_law_analysis.invoke(
                ...
                print(
                    f"在 analyze_case_with_law 函数的 generate_law_analysis 中调用模型时出错: {e}"
                )
                return None
            history_control.update_session_history(
                input_user_id, input_session_id, case_description, law_analysis.content
            )
            return law_analysis
        ...
        def generate_final_analysis():
            ...
            <|history|>上下文记录:{{history}}<|end|>
            """
            prompt_final_analysis = ChatPromptTemplate.from_template(
                template_final_analysis
            )
            history_control = ControlChatHistoryData()
            ...
            try:
                final_analysis = chat_with_history_final_analysis.invoke(
                    {"question": case_description},
                    config={
                        "input_user_id": input_user_id,
                        "input_session_id": input_session_id,
                ...
                print(
                    f"在 analyze_case_with_law 函数的 generate_final_analysis 中调用模型时出错: {e}"
                )
                return None, None
            history_control.update_session_history(
                input_user_id,
